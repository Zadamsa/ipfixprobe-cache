...
static std::string get_alpn_label(const TLSParser& parser)
{
    ...
    if (parser.get_alpns().empty() || parser.get_alpns()[0].second == 0){
        alpn_label = "00";
    } else {
        const auto& alpn_string = parser.get_alpns()[0];
        alpn_label += convert_alpn_byte_to_label(alpn_string.first[0], true);
        alpn_label += convert_alpn_byte_to_label(alpn_string.first  [alpn_string.second - 1], false);
    }
    return alpn_label;
}
...
{
   TLSParser parser;
   if (!parser.parse(data, payload_len)){
       return false;
   }
